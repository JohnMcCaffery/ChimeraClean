...
namespace Chimera.OpenSim.Packets {
    /// <exclude/>
    public sealed class ClearFrustumPacket : Packet
    {
        /// <exclude/>
        ...
        public ClearFrustumPacket(UUID id)
            : this() {
            ObjectData.ObjectID = id;
        }

        public ClearFrustumPacket(byte[] bytes, ref int i) : this()
        {
        ...
        public ClearFrustumPacket(Header head, byte[] bytes, ref int i): this()
        {
            int packetEnd = bytes.Length - 1;
