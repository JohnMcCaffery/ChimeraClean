...
namespace Chimera.Plugins {
    class ShutdownPlugin : ISystemPlugin {
        #region ISystemPlugin Members

        public void Init(Core core) {
            mCore = core;
            config = new ShutdownConfig();
            Key = config.ShutdownKey;
            log = LogManager.GetLogger("Shutdown");
        ...
        private void mCoordinator_KeyUp(Core coord, KeyEventArgs args) {
                ...
                log.InfoFormat("Modifier {0} Key {1}", args.Modifiers, args.KeyData);
                if (args.Modifiers == Keys.None && args.KeyData == Key) {
                    if (mForm != null) {
                        mForm.Close();
                    }
                }
    ...
    public class Hotkey : IMessageFilter {
        ...
        public void Unregister() {
            ...
            if (!this.windowControl.IsDisposed) {
                ...
                if (Hotkey.UnregisterHotKey(this.windowControl.Handle, this.id) == 0) { throw new Win32Exception(); }
            }
